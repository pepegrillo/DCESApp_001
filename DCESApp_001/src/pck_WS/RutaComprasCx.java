...
public class RutaComprasCx {
	...
	RutaComprasSG rutacompras = new RutaComprasSG();
	
	public RutaComprasCx(String IdListaReturn){
		
		try{
			URI uri = URI.create(path.Path());
			Database sqliteDB = DatabaseFactory.open(uri);
			
			Statement se = sqliteDB.createStatement(statement.SelectRutaComprasValidar(IdListaReturn));
			se.prepare();
	...
	public void descargarDatos(String IdListaReturn, String hashKey){
		...
		try{
			...
			connectionURL = Strings.HTTP_SW+"getEstablecimientosAgregadosRuta/"+IdListaReturn+"/"+hashKey+tipoConexion;
	            	...
	            	for (int i = 0; i < jsonMainArr.length(); i++) {
		            	...
		            	try{
		            		...
		            		Statement in = sqliteDB1.createStatement(
		            				statement.InsertRutaCompras(childJSONObject.getString("idEstablecimiento"),
	...
	public void cargarDatos(String IdListaReturn){
		...
		try{
			...
			Statement selectR = sqliteDB.createStatement(statement.SelectRutaComprasValidar(IdListaReturn));
			selectR.prepare();
			...
			while(cursorR.next()){
				rc = cursorR.getRow();
				IdEstablecimiento.addElement(rc.getString(0));
				NombreEstablecimiento.addElement(rc.getString(1));
				//Dialog.alert("SQL<"+rc);
			}
			...
			rutacompras.setIdEstablecimiento(IdEstablecimiento);
			rutacompras.setNombreEstablecimiento(NombreEstablecimiento);
			
			rutacompras.seterrorCode(errorCode);
			rutacompras.seterrorMessage(errorMessage);
			
			rutacompras.sethashKey(hashKey);
		}
	...
	public void eliminarDatos(String idLista){
		...
		try{
        	URI uri1 = URI.create(path.Path());
			Database sqliteDB1 = DatabaseFactory.open(uri1);
			Statement dt = sqliteDB1.createStatement(statement.DeleteRutaCompras(idLista));
	...
	public void eliminarEstablecimiento(String idLista, String idEstablecimiento){
		...
		try{
			...
			connectionURL = Strings.HTTP_SW+"deleteEstablecimientoLista/"+hashKey+"/"+idLista+"/"+idEstablecimiento+tipoConexion;
	        ...
	        if(conn.getResponseCode() == HttpConnection.HTTP_OK)
	        {
	            ...
	            if(errorCode.equals("0")){
	            	Status.show("Establecimiento eliminado correctamente.");

...
public class ProductoCompras extends Estilos implements FieldChangeListener {
	...
	public ProductoCompras(String idLista) {
		...
		try {
			...
			VerticalField allContentListaCategoria = new VerticalField(Display.getWidth(),trecientoscuarentaysiete,HorizontalField.FIELD_HCENTER | VerticalField.VERTICAL_SCROLL | VerticalField.VERTICAL_SCROLLBAR);
				...
				for (int i = 0; i < productocompras.IdArticulo.size(); i++){
					...
					vLista.addElement(new ListStyleButtonField(arrow,""+productocompras.NombreArticulo.elementAt(i) , null,DrawStyle.ELLIPSIS){
				...
				errorHfm.setBackground(BackgroundFactory.createBitmapBackground(Bitmap.getBitmapResource(productocompras.errorMessage)));
		...
		// Menu Item
		MenuItem myItem = new MenuItem(new StringProvider("Agregar productos"), 110, 0){
		    public void run(){
		    	
		    	UiApplication.getUiApplication().pushScreen(new BuscarProductoCompras(IdLista));

...
import pck_WS.BusquedaProductoComprasCx;
...
public class ResultadoBusquedaProductoCompras extends Estilos implements FieldChangeListener {
	...
	int anchoDatoExtra = 200;
	
	BusquedaProductoComprasCx busquedaproductocompras;
	
	
	public ResultadoBusquedaProductoCompras(String idLista, String txtProductoName) {
		
		IdLista = idLista;
		
		busquedaproductocompras = new BusquedaProductoComprasCx(IdLista,txtProductoName);
		...
		try {
			...
			ColorRichText emailCrt = new ColorRichText("Productos de compras",0xffffff, RichTextField.FIELD_HCENTER | RichTextField.TEXT_ALIGN_HCENTER);
			...
			if (busquedaproductocompras.errorCode.equals("0")){
			//Lista Categoria
			VerticalField allContentListaCategoria = new VerticalField(Display.getWidth(),trecientoscuarentaysiete,HorizontalField.FIELD_HCENTER | VerticalField.VERTICAL_SCROLL | VerticalField.VERTICAL_SCROLLBAR);
				...
				for (int i = 0; i < busquedaproductocompras.IdArticulo.size(); i++){
					...
					vLista.addElement(new ListStyleButtonField(arrow,""+busquedaproductocompras.NombreArticulo.elementAt(i) , null,DrawStyle.ELLIPSIS){
		    	    ...
		    	    HorizontalFieldManager extras = new HorizontalFieldManager();
		    	    
		    	    LabelField datoextra = new LabelField(" - "+busquedaproductocompras.PresentacionArticulo.elementAt(i).toString(),DrawStyle.ELLIPSIS | LabelField.FIELD_LEFT){
		    	    	public int getPreferredWidth(){return Display.getWidth()-anchoDatoExtra;}
		    	    	public void paint(Graphics g)
						{      
							g.setColor(0x1c6dae);
							super.paint(g);
		    	    ...
		    	    LabelField datoextra2 = new LabelField(" Marca: "+busquedaproductocompras.MarcaArticulo.elementAt(i).toString(),DrawStyle.ELLIPSIS | LabelField.FIELD_LEFT){
		    	    ...
		    	    extras.add(datoextra);
		    	    extras.add(datoextra2);
		    	    
		    	    contentListCategoria.add(extras);
				...
				errorHfm.setBackground(BackgroundFactory.createBitmapBackground(Bitmap.getBitmapResource(busquedaproductocompras.errorMessage)));
	...
	public void fieldChanged(Field field, int context) {
		...
		for(int j=0;j<=vLista.size()-1;j++){
			if( vLista.elementAt(j)== field ){
				options(IdLista,busquedaproductocompras.IdArticulo.elementAt(j).toString());
	...
	public void options(final String idLista, final String IdArticulo){
    	UiApplication.getUiApplication().invokeLater(new Runnable(){
    		...
    		public void run(){
				...
				int result = Dialog.ask("Agregar Producto ", choices, 0);
				switch (result) {
				case 0:
					busquedaproductocompras.guardarProductoCompras(idLista,IdArticulo);

...
import listaCompras.ConfigurarListaCompras;
...
public class RutaCompras extends Estilos implements FieldChangeListener {
	...
	RutaComprasCx rutacompras;
	
	public RutaCompras(final String IdListaReturn) {
		
		idLista = IdListaReturn;
		
		rutacompras = new RutaComprasCx(IdListaReturn);
		...
		try {
			...
			ColorRichText emailCrt = new ColorRichText("Ruta de compras",0xffffff, RichTextField.FIELD_HCENTER | RichTextField.TEXT_ALIGN_HCENTER);
			...
			if (rutacompras.errorCode.equals("0")){
			//Lista Categoria
			VerticalField allContentListaCategoria = new VerticalField(Display.getWidth(),trecientoscuarentaysiete,HorizontalField.FIELD_HCENTER | VerticalField.VERTICAL_SCROLL | VerticalField.VERTICAL_SCROLLBAR);
				...
				for (int i = 0; i < rutacompras.IdEstablecimiento.size(); i++){
					...
					vLista.addElement(new ListStyleButtonField(null,""+rutacompras.NombreEstablecimiento.elementAt(i) , null,DrawStyle.ELLIPSIS){
				...
				errorHfm.setBackground(BackgroundFactory.createBitmapBackground(Bitmap.getBitmapResource(rutacompras.errorMessage)));
		...
		// Menu Item
		MenuItem myItem = new MenuItem(new StringProvider("Agregar establecimientos"), 110, 0){
		    public void run(){
		    	
		    	UiApplication.getUiApplication().pushScreen(new MunicipioCompras(IdListaReturn));
	...
	public void fieldChanged(Field field, int context) {
		...
		for(int j=0;j<=vLista.size()-1;j++){
			if( vLista.elementAt(j)== field ){
				options(idLista,rutacompras.IdEstablecimiento.elementAt(j).toString());
			}
		}
	}
	...
	public void options(final String idLista, final String idEstablecimiento){
    	UiApplication.getUiApplication().invokeLater(new Runnable(){
    		...
    		public void run(){
				...
				int result = Dialog.ask("Eliminar Establecimiento ", choices, 0);
				switch (result) {
				case 0:
					rutacompras.eliminarEstablecimiento(idLista,idEstablecimiento);

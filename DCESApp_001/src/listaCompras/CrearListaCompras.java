...
public class CrearListaCompras extends Estilos {
	...
	public CrearListaCompras() {
		...
		try {
			...
			ColorRichText emailCrt = new ColorRichText("Nueva lista",0xffffff, RichTextField.FIELD_HCENTER | RichTextField.TEXT_ALIGN_HCENTER);
			...
			ColorRichText lastnameCrt = new ColorRichText("INGRESE EL NOMBRE DE LA LISTA:", 0x9cbe4f, RichTextField.FIELD_LEFT | RichTextField.TEXT_ALIGN_LEFT);
			lastnameCrt.setFont(fLite);
			lastnameCrt.setMargin(veinte, 20, cinco, cincuenta);
			allContentDetalle.add(lastnameCrt);

			txtListName = new BasicEditField("", "", 40, BasicEditField.JUMP_FOCUS_AT_END | TextField.NO_NEWLINE) {
				public void paint(Graphics g) {
					g.setColor(0xFFF);
					super.paint(g);
				}
			};
			txtListName.setBorder(BorderFactory.createBitmapBorder(new XYEdges(5,13, 5, 13), bordes));
			txtListName.setMargin(0, cincuenta, 0, cincuenta);
			txtListName.setPadding(cinco, cinco, cinco, cinco);
			allContentDetalle.add(txtListName);
			...
			btnAceptarRegUser.setChangeListener(new FieldChangeListener() {
				public void fieldChanged(Field field, int context) {
					if (!txtListName.getText().toString().equals("")){
						crearlistacompras.postListaCompras(txtListName.getText().toString());
						UiApplication.getUiApplication().pushScreen(new ConfigurarListaCompras(crearlistacompras.IdListaReturn));
					}else{
						Status.show("Debes asignarle un nombre a la nueva lista de compras.",Bitmap.getPredefinedBitmap(Bitmap.INFORMATION),3000);
					}
